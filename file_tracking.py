...
def confidence_to_color(confidence: float) -> Tuple[int, int, int]:
    # clamp value within range 0-100
    value = max(0, min(100, confidence*100)) # confidence is 0-1
    ...
    if value < 50:
        ...
        # between yellow (255,255,0) and green (0,255,0)
        r = int(255 * (1 - (value - 50) / 50))
        g = 255
        b = 0

    return (b, g, r) # opencv BGR convention
...
def main() -> None:
    ...
    video_path = "videos/animals3_compressed_cropped_30.mp4"  # Replace with the path to your video file
    ...
    while cap.isOpened():
        ...
        for box in boxes:
            ...
            cv2.rectangle(frame, (int(x1),int(y1)), (int(x2),int(y2)), confidence_to_color(confidence), 2)
            ...
            padding_size = 3
            # label background
            cv2.rectangle(frame, (int(x1)-1, int(y1-text_height)-2*padding_size), (int(x1+text_width), int(y1)), confidence_to_color(confidence), thickness=cv2.FILLED)

            # draw white label text
            cv2.putText(frame, text, (int(x1), int(y1)-padding_size), cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255, 255, 255), thickness=2)
